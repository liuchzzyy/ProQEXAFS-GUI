...
class DataExtract(tkinter.Frame):
    def __init__(self, name, *args, **kwargs):
        ...
        self.progress_bar = ttk.Progressbar(
            self, orient="horizontal", mode="determinate", maximum=100, style="red.Horizontal.TProgressbar"
        )
        ...
        self.frame1 = tkinter.LabelFrame(
            self, width=0.25 * 0.8333 * self.screen_height, height=0.75 * self.screen_height
        )
        self.frame2 = tkinter.LabelFrame(
            self, width=0.7 * 0.8333 * self.screen_height, height=0.7 * 0.75 * self.screen_height
        )
        self.frame3 = tkinter.LabelFrame(
            self,
            width=0.7 * 0.8333 * self.screen_height,
            height=0.3 * 0.75 * self.screen_height,
            borderwidth=0,
            highlightthickness=0,
        )
        self.frame4 = tkinter.LabelFrame(
            self,
            width=0.25 * 0.8333 * self.screen_height,
            height=0.75 * self.screen_height,
            borderwidth=0,
            highlightthickness=0,
        )
        self.frame5 = tkinter.LabelFrame(
            self,
            width=0.7 * 0.8333 * self.screen_height,
            height=0.3 * 0.75 * self.screen_height,
            borderwidth=0,
        ...
        self.sam_logCheck = tkinter.Checkbutton(
            self.plotcolsFrame, variable=self.sam_logvar, command=self.plot_update_trigger
        )
        self.sam_logCheck.grid(column=1, row=3)

        self.ref_logvar = tkinter.DoubleVar()
        self.ref_logvar.set(1)
        self.ref_logCheck = tkinter.Checkbutton(
            self.plotcolsFrame, variable=self.ref_logvar, command=self.plot_update_trigger
        )
        ...
        self.FilterCheck = tkinter.Checkbutton(
            self.plotcolsFrame, variable=self.Filtervar, command=self.plot_update_trigger
        )
        ...
        for idx, (text, mode) in enumerate(toggle):
            self.radio_plot_buttons.append(
                tkinter.Radiobutton(
                    self.plotcolsFrame, text=text, variable=self.togglevar, value=mode, command=self.plot_update_trigger
                )
            )
            self.radio_plot_buttons[-1].grid(row=4, column=idx + 1)

        self.plot_update_button = tkinter.Button(
            self.plotcolsFrame, text="Update Plot", command=self.plot_update_trigger
        ).grid(column=0, row=6)
        ...
        # FRAME4-1
        ################################################################################
        self.ToProcessFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=21, padx=123
        )
        tkinter.Button(self.ToProcessFrame, text="Go to Data Export", command=lambda: raise_frame(f2)).grid(
            row=0, column=0, columnspan=3
        )

        ################################################################################
        # Encoder Analysis Frame
        self.EncoderFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=5
        )
        self.EncoderFrame.grid(row=1, column=0)

        self.split_accept_button = tkinter.Button(
            self.EncoderFrame, text="Accept", state="disabled", width=14, command=self.accept_split
        ).grid(column=0, row=5, stick="E")
        self.split_insert_button = tkinter.Button(
            self.EncoderFrame, text="Insert", state="disabled", width=14, command=self.insert_split
        ).grid(column=1, row=5, stick="EW")
        self.split_delete_button = tkinter.Button(
            self.EncoderFrame, text="Delete", state="disabled", width=14, command=self.delete_split
        ).grid(column=2, row=5, stick="W")

        self.buttonsplit = tkinter.Button(
            self.EncoderFrame, text="Analyze Encoder", width=30, height=1, command=self.split
        )
        ...
        ################################################################################
        # Calibration Frame
        self.CalibrationFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=28
        )

        self.buttoncalib = tkinter.Button(
            self.CalibrationFrame, text="Calibrate", width=30, height=1, command=self.calibrate
        )
        ...
        self.calibmonoMenu = tkinter.OptionMenu(
            self.CalibrationFrame, self.calibmono, "Si 111", "Si 111 LN", "Si 311", "Si 311 LN"
        )
        ...
        self.CalibUseCheck = tkinter.Checkbutton(
            self.CalibrationFrame, variable=self.CalibUsevar, command=self.plot_update_trigger
        )
        ...
        self.BlackmanHarrisFiltervarCheck = tkinter.Checkbutton(
            self.CalibrationFrame, variable=self.BlackmanHarrisFiltervar
        )
        ...
        self.calib_accept_button = tkinter.Button(
            self.CalibrationFrame, text="Accept", state="disabled", width=14, command=self.accept_calib
        ).grid(column=1, row=9, stick="E")
        self.calib_cancel_button = tkinter.Button(
            self.CalibrationFrame, text="Cancel", state="disabled", width=14, command=self.cancel_calib
        ).grid(column=2, row=9, stick="W")

        ################################################################################
        # Normalization Frame
        self.NormalisationFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=11
        )

        self.buttonnorm = tkinter.Button(
            self.NormalisationFrame, text="Normalise", width=30, height=1, command=self.normalize
        )
        ...
        self.NormUseCheck = tkinter.Checkbutton(
            self.NormalisationFrame, variable=self.NormUsevar, command=self.plot_update_trigger
        )
        ...
        self.Flatvar.set(1)
        self.FlatCheck = tkinter.Checkbutton(
            self.NormalisationFrame, variable=self.Flatvar, command=self.plot_update_trigger
        )
        ...
        self.buttonloadnorm = tkinter.Button(
            self.NormalisationFrame, text="Load Normalization", width=14, height=1, command=self.normload
        )
        self.buttonloadnorm.grid(column=2, row=1, columnspan=1)

        self.norm_pre = tkinter.StringVar(self.NormalisationFrame)
        self.norm_pre.set("1")
        self.norm_preMenu = tkinter.OptionMenu(
            self.NormalisationFrame, self.norm_pre, "0", "1", "2", "3", "4", "V", command=self.plot_update_trigger2
        )
        ...
        self.norm_postMenu = tkinter.OptionMenu(
            self.NormalisationFrame, self.norm_post, "0", "1", "2", "3", "4", "V", command=self.plot_update_trigger2
        )
        self.norm_postMenu.grid(column=2, row=7, columnspan=1)

        self.pre1_scroll_scale = tkinter.Scale(
            self.NormalisationFrame,
            orient="horizontal",
            from_=-150,
            to=0,
            state="disabled",
        ...
        self.pre2_scroll_scale = tkinter.Scale(
            self.NormalisationFrame,
            orient="horizontal",
            from_=-150,
            to=0,
        ...
        self.post1_scroll_scale = tkinter.Scale(
            self.NormalisationFrame,
            orient="horizontal",
            from_=0,
            to=1000,
        ...
        self.post2_scroll_scale = tkinter.Scale(
            self.NormalisationFrame,
            orient="horizontal",
            from_=0,
            to=1000,
        ...
        self.norm_accept_button = tkinter.Button(
            self.NormalisationFrame, text="Accept", state="disabled", width=14, command=self.accept_norm
        ).grid(column=1, row=11, stick="E")
        self.norm_cancel_button = tkinter.Button(
            self.NormalisationFrame, text="Cancel", state="disabled", width=14, command=self.cancel_norm
        ).grid(column=2, row=11, stick="W")

        self.update_e0_button = tkinter.Button(self.NormalisationFrame, text="Update E0").grid(column=2, row=9)

        ################################################################################
        # Interpolation Frame
        self.InterpolationFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=64
        )
        ...
        self.InterpUseCheck = tkinter.Checkbutton(
            self.InterpolationFrame, variable=self.InterpUsevar, command=self.plot_update_trigger
        )
        ...
        self.constkUseCheck = tkinter.Checkbutton(
            self.InterpolationFrame, variable=self.constkUsevar, command=self.plot_update_trigger
        )
        ...
        for idx, (text, mode) in enumerate(toggle):
            self.radio_updown_buttons.append(
                tkinter.Radiobutton(self.exportFrame, text=text, variable=self.updownvar, value=mode)
            )
        ...
        self.buttonexport = tkinter.Button(
            self.BatchFrame, text="Batch Export", width=30, height=1, command=self.batch_export
        )
        ...
        self.ds_scroll_scale = tkinter.Scale(
            self.scrollFrame, orient="horizontal", from_=0, to=1, state="disabled", width=18
        )
        ...
        self.bf_scroll_scale = tkinter.Scale(
            self.scrollFrame,
            orient="horizontal",
            from_=0.0005,
            to=0.5,
        ...
        self.cp_scroll_scale = tkinter.Scale(
            self.scrollFrame,
            orient="horizontal",
            from_=0,
            to=1,
    ...
    def edge_box(self, file):
        ...
        self.box.diffcalibUseCheck = tkinter.Checkbutton(
            self.box,
            variable=self.box.diffcalibUsevar,
            command=lambda: self.naccheck(self.box, self.box.diffcalibUsevar),
        )
    ...
    def onselect(self, evt):
        ...
        if "Data Extract" in App.get_selected(self):
            ...
            self.Fbutter = 1.5 * (
                self.e0_scroll_scale.get()
                - (
                    1239.852
                    / (
                        np.sin(
                            (
                                (np.arcsin(1239.852 / (self.twod.get() * self.e0_scroll_scale.get())) * 180 / np.pi)
                                + 0.00005
                            )
                            * np.pi
                            / 180
    ...
    def normframe_load_values(self):
        self.pre1, self.pre2, self.post1, self.post2, self.order_pre, self.order_post, self.e0 = (
        ...
        self.pre1_scroll_scale = tkinter.Scale(
            self.NormalisationFrame,
            orient="horizontal",
            from_=np.min(self.energy) - float(self.e0) + 5,
            to=0,
            state="normal",
            digits=4,
            resolution=0.1,
        ...
        self.post1_scroll_scale = tkinter.Scale(
            ...
            orient="horizontal",
            from_=0,
            to=np.max(self.energy) - float(self.e0) - 10,
            state="normal",
            digits=4,
            resolution=0.1,
            length=125,
    ...
    def Split_buttons_visible(self):
        self.split_accept_button = tkinter.Button(
            self.EncoderFrame, text="Accept", state="normal", width=14, command=self.accept_split
        ).grid(column=0, row=5, stick="E")
        self.split_insert_button = tkinter.Button(
            self.EncoderFrame, text="Insert", state="normal", width=14, command=self.insert_split
        ).grid(column=1, row=5, stick="EW")
        self.split_delete_button = tkinter.Button(
            self.EncoderFrame, text="Delete", state="normal", width=14, command=self.delete_split
        ).grid(column=2, row=5, stick="W")
    ...
    def header_read_qex(self, filename):
        ...
        self.line_bytes = int(
            sum([int(format_item[1]) for format_item in [re.split(r"(\d+)", s) for s in DataLineFormat]]) / 8
        )
    ...
    def data_read_qex(self, root_i, minr):
        ...
        if self.BlackmanHarrisFiltervar.get() == 1:
            RawData = RawData.rolling(
                int(self.blackmanfilterwindowvar.get()), win_type="blackmanharris", min_periods=1, center=True
            ).mean()
            RawData.dropna(how="any", inplace=True)
    ...
    def plot_add_markerlines(self):
        ...
        [self.pre1line] = self.ax.plot(
            [
                self.e0_scroll_scale.get() + self.pre1_scroll_scale.get(),
                self.e0_scroll_scale.get() + self.pre1_scroll_scale.get(),
            ],
            ylims,
            ls="--",
        ...
        [self.pre2line] = self.ax.plot(
            [
                self.e0_scroll_scale.get() + self.pre2_scroll_scale.get(),
                self.e0_scroll_scale.get() + self.pre2_scroll_scale.get(),
            ],
            ylims,
            ls="--",
        ...
        [self.post1line] = self.ax.plot(
            [
                self.e0_scroll_scale.get() + self.post1_scroll_scale.get(),
                self.e0_scroll_scale.get() + self.post1_scroll_scale.get(),
            ],
            ylims,
            ls="--",
        ...
        [self.post2line] = self.ax.plot(
            [
                self.e0_scroll_scale.get() + self.post2_scroll_scale.get(),
                self.e0_scroll_scale.get() + self.post2_scroll_scale.get(),
            ],
            ylims,
            ls="--",
        ...
        def plot_data(evt):
            ...
            if self.trigger == 0:
                ...
                if (self.calibrated == True) & (self.CalibUsevar.get() == 1):
                    self.calibration_values = (
                        pd.read_csv(self.folder + "/calibration.dat", sep="\t", header=None)
                    ).values
                    self.energy = 1239.852 / (
                        float(self.calibration_values[3])
                        * np.sin((ang + (self.calibration_values[1] - self.calibration_values[0])) * np.pi / 180)
                    )
                    if self.Filtervar.get() == 1:
                    ...
                    if (self.normalized == True) & (self.NormUsevar.get() == 1):
                        self.normalisation_values = pd.read_csv(
                            self.folder + "/normalisation.dat", sep="\t", header=None
                        ).values
                        self.pre1, self.pre2, self.post1, self.post2, self.order_pre, self.order_post, self.e0 = (
                            self.normalisation_values
                        )
            ...
            if self.trigger == 1:
                ...
                if (direction_flag == 1) or (self.naveragewindowvar.get() > 1):
                    try:
                        if ".bin" in self.file_type:
                            for i in range(int(self.naveragewindowvar.get())):
                                if i == 0:
                                    if direction_flag == 1:
                                    ...
                                    if direction_flag_updated == 1:
                                        ang_cal, mu_cal, std, direction_flag_cal = self.data_read_bin(
                                            root_i + 1 + i, self.minr
                                        )
                                    else:
                                        ang_cal, mu_cal, std, direction_flag_cal = self.data_read_bin(
                                            root_i + i, self.minr
                        ...
                        if ".qex" in self.file_type:
                            for i in range(int(self.naveragewindowvar.get())):
                                if i == 0:
                                    ...
                                    if direction_flag_updated == 1:
                                        ang_cal, mu_cal, direction_flag_cal = self.data_read_qex(
                                            root_i + 1 + i, self.minr
                                        )
                                    else:
            ...
            if self.trigger == 2:
                ...
                self.normalisation_values = [
                    self.pre1_scroll_scale.get(),
                    self.pre2_scroll_scale.get(),
                    self.post1_scroll_scale.get(),
                    self.post2_scroll_scale.get(),
                    self.norm_pre.get(),
                    self.norm_post.get(),
                    self.e0_scroll_scale.get(),
                ]
                ...
                if self.initialize_plot == 0:
                    ...
                    ylims = self.ax.get_ylim()
                    self.pre1line.set_xdata([
                        self.e0_scroll_scale.get() + self.pre1_scroll_scale.get(),
                        self.e0_scroll_scale.get() + self.pre1_scroll_scale.get(),
                    ])
                    self.pre2line.set_xdata([
                        self.e0_scroll_scale.get() + self.pre2_scroll_scale.get(),
                        self.e0_scroll_scale.get() + self.pre2_scroll_scale.get(),
                    ])
                    self.post1line.set_xdata([
                        self.e0_scroll_scale.get() + self.post1_scroll_scale.get(),
                        self.e0_scroll_scale.get() + self.post1_scroll_scale.get(),
                    ])
                    self.post2line.set_xdata([
                        self.e0_scroll_scale.get() + self.post2_scroll_scale.get(),
                        self.e0_scroll_scale.get() + self.post2_scroll_scale.get(),
                    ])
                    self.canvas.draw_idle()
                ...
                if self.trigger == 2:
                    ...
                    def scroll_bar_update(*args):
                        self.pre1_scroll_scale = tkinter.Scale(
                            self.NormalisationFrame,
                            orient="horizontal",
                            from_=np.min(self.energy) - self.e0_scroll_scale.get() + 5,
                            to=0,
                            state="normal",
                            digits=4,
                            resolution=0.1,
                        ...
                        self.post1_scroll_scale = tkinter.Scale(
                            ...
                            orient="horizontal",
                            from_=0,
                            to=np.max(self.energy) - self.e0_scroll_scale.get() - 10,
                            state="normal",
                            digits=4,
                            resolution=0.1,
                            length=125,
                        ...
                        self.e0_entry = tkinter.Entry(
                            self.NormalisationFrame, textvariable=self.e0_scroll_scale, state="normal"
                        )
                        self.update()

                        self.update_e0_button = tkinter.Button(
                            self.NormalisationFrame, text="Update E0", command=callback
                        ).grid(column=2, row=9)
    ...
    def calibrate(self):
        ...
        self.calib_accept_button = tkinter.Button(
            self.CalibrationFrame, text="Accept", state="normal", width=14, command=self.accept_calib
        ).grid(column=1, row=9, stick="E")
        self.calib_cancel_button = tkinter.Button(
            self.CalibrationFrame, text="Cancel", state="normal", width=14, command=self.cancel_calib
        ).grid(column=2, row=9, stick="W")

        self.cp_scroll_scale = tkinter.Scale(
            self.scrollFrame,
            orient="horizontal",
            from_=float(self.min_ang),
            to=float(self.max_ang),
            digits=7,
            resolution=0.000025,
            state="normal",
            width=18,
            command=self.plot_cp_line,
        )
        self.cp_scroll_scale.grid(column=1, columnspan=1, row=2, stick="EW")
    ...
    def accept_calib(self):
        ...
        calibvalue = [
            self.cp_scroll_scale.get(),
            self.thetac,
            float(self.bf_scroll_scale.get()),
            self.twod.get(),
            self.calibenervar.get(),
        ]
    ...
    def normalize(self):
        ...
        self.norm_accept_button = tkinter.Button(
            self.NormalisationFrame, text="Accept", state="normal", width=14, command=self.accept_norm
        ).grid(column=1, row=11, stick="E")
        self.norm_cancel_button = tkinter.Button(
            self.NormalisationFrame, text="Cancel", state="normal", width=14, command=self.cancel_norm
        ).grid(column=2, row=11, stick="W")
    ...
    def batch_export(self):
        ...
        if (
            (self.AutoAlignUsevar.get() == 1)
            & (self.firstcalibvar.get() == 1)
            & (self.calibrated == True)
            & (self.normalized == True)
            & (self.NormUsevar.get() == 1)
            & (self.CalibUsevar.get() == 1)
        ):
        ...
        if proceed_var == True:
            self.update()
            options = [
                int(len(self.calibroots) - 5),
                self.data_file,
                self.folder,
                self.file_type,
                num_cpus,
                self.ProcessSamUsevar.get(),
                self.ProcessRefUsevar.get(),
                self.NormUsevar.get(),
                self.InterpUsevar.get(),
                self.AutoAlignUsevar.get(),
                self.edgestepvar.get(),
                self.Filtervar.get(),
                self.bf_scroll_scale.get(),
                self.eminvar.get(),
                self.emaxxanesvar.get(),
                self.emaxexafsvar.get(),
                self.constkUsevar.get(),
                self.kstepvar.get(),
                self.estepvar.get(),
                str(self.sam_numer.get()),
                str(self.sam_denom.get()),
                int(self.sam_logvar.get()),
                str(self.ref_numer.get()),
                str(self.ref_denom.get()),
                int(self.ref_logvar.get()),
                int(self.Flatvar.get()),
                self.updownvar.get(),
                self.edgestepwidth,
                self.BlackmanHarrisFiltervar.get(),
                self.blackmanfilterwindowvar.get(),
            ]
            break_flag = False
        ...
        for i in range(num_cpus):
            if self.updownvar.get() == 0:
                if self.ProcessSamUsevar.get() == 1:
                    data_read_sample = pd.read_csv(
                        self.folder + "/Export/Merged_Up/sample_" + str(int(i)) + ".dat", sep="\t", header=0
                    )
                if self.ProcessRefUsevar.get() == 1:
                    data_read_reference = pd.read_csv(
                        self.folder + "/Export/Merged_Up/reference_" + str(int(i)) + ".dat", sep="\t", header=0
                    )
            if self.updownvar.get() == 1:
                if self.ProcessSamUsevar.get() == 1:
                    data_read_sample = pd.read_csv(
                        self.folder + "/Export/Merged_Down/sample_" + str(int(i)) + ".dat", sep="\t", header=0
                    )
                if self.ProcessRefUsevar.get() == 1:
                    data_read_reference = pd.read_csv(
                        self.folder + "/Export/Merged_Down/reference_" + str(int(i)) + ".dat", sep="\t", header=0
                    )
            if self.updownvar.get() == 2:
                if self.ProcessSamUsevar.get() == 1:
                    data_read_sample = pd.read_csv(
                        self.folder + "/Export/Merged_Both/sample_" + str(int(i)) + ".dat", sep="\t", header=0
                    )
                if self.ProcessRefUsevar.get() == 1:
                    data_read_reference = pd.read_csv(
                        self.folder + "/Export/Merged_Both/reference_" + str(int(i)) + ".dat", sep="\t", header=0
                    )
            if i == 0:
                if self.ProcessSamUsevar.get() == 1:
        ...
        try:
            if self.ProcessSamUsevar.get() == 1:
                df_sam.to_csv(
                    self.folder + "/Export/" + self.data_file.split("/")[-1] + "_sam_matrix_" + updownstring + ".dat",
                    sep="\t",
                    index=False,
                )
                # if '.bin' in self.file_type:
        ...
        try:
            if self.ProcessRefUsevar.get() == 1:
                df_ref.to_csv(
                    self.folder + "/Export/" + self.data_file.split("/")[-1] + "_ref_matrix_" + updownstring + ".dat",
                    sep="\t",
                    index=False,
                )
                # if '.bin' in self.file_type:
...
class PostProcess(tkinter.Frame):
    def __init__(self, name, *args, **kwargs):
        ...
        self.frame2 = tkinter.LabelFrame(
            self, width=0.5 * 0.8333 * self.screen_height, height=0.7 * 0.75 * self.screen_height
        )
        self.frame3 = tkinter.LabelFrame(
            self,
            width=0.25 * 0.8333 * self.screen_height,
        ...
        ################################################################################
        # Average Frame
        self.AverageFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=73
        )
        ...
        self.AverageUsevarCheck = tkinter.Checkbutton(
            self.AverageFrame, variable=self.AverageUsevar, command=self.plot_update_trigger
        )
        ...
        self.buttonaverage = tkinter.Button(
            self.AverageFrame, text="Average", width=30, height=1, command=self.average_trigger
        )
        self.buttonaverage.grid(column=0, row=7, columnspan=3, rowspan=1)

        self.saveaverage = tkinter.Button(
            self.AverageFrame, text="Save Average", width=30, height=1, command=self.saveaverage_trigger
        )
        ...
        # Normalization Frame
        self.NormalisationFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=15
        )
        ...
        self.save_norm_data_button = tkinter.Button(
            self.NormalisationFrame, text="Save Normalised Data", state="normal", width=28, command=self.save_norm_data
        ).grid(column=1, row=12, columnspan=2, stick="EW")
        ...
        ################################################################################
        # Time Filter Frame
        self.TimeFrame = tkinter.LabelFrame(
            master=self.f1, width=300, height=0.75 * self.screen_height, pady=5, padx=75
        )
        ...
        self.TimeFilterUsevarCheck = tkinter.Checkbutton(
            self.TimeFrame, variable=self.TimeFilterUsevar, command=self.plot_update_trigger
        )
        ...
        self.cutusevarCheck = tkinter.Checkbutton(
            self.CutFrame, variable=self.cutusevar, command=self.plot_update_trigger
        )
    ...
    def average_trigger(self):
        ...
        if Nave > 0:
            ...
            for j in range(int(Nave)):
                if startval == True:
                    if int(idxs[1]) - int(idxs[0]) == 2:
                        idx = int(self.nstartvar.get()) + npi.indices(
                            np.asarray(idxs),
                            np.asarray(idxs)[
                                (np.asarray(idxs) >= ((j * Nsum) + 2 * int(self.nstartvar.get())))
                                & (np.asarray(idxs) < ((j * Nsum) + Nsum + 2 * int(self.nstartvar.get())))
                            ],
                        )
                        # print(j, idx)
                    else:
                        idx = int(self.nstartvar.get()) + npi.indices(
                            np.asarray(idxs),
                            np.asarray(idxs)[
                                (np.asarray(idxs) >= ((j * Nsum) + int(self.nstartvar.get())))
                                & (np.asarray(idxs) < ((j * Nsum) + Nsum + int(self.nstartvar.get())))
                            ],
                        )
                        # print(j, idx)
                else:
                    idx = npi.indices(
                        np.asarray(idxs),
                        np.asarray(idxs)[(np.asarray(idxs) >= (j * Nsum)) & (np.asarray(idxs) < (j * Nsum) + Nsum)],
                    )
                ...
                if len(idx) > 0:
                    ...
                    if self.errors_flag == True:
                        self.data_sum_error[str(j)] = (self.data_error.iloc[:, idx + 1]).mean(axis=1) / np.sqrt(
                            len(idx)
                        )
            ...
            if self.TimeFilterUsevar.get() == 1:
                ...
                if Nave > self.windowvar.get():
                    ...
                    for i in range(self.data_sum.shape[0]):
                        self.data_sum.iloc[[i]] = signal.savgol_filter(
                            self.data_sum.iloc[[i]], int(self.windowvar.get()), int(self.ordervar.get())
                        )
                else:
            ...
            if self.cutusevar.get() == 1:
                self.data_sum.drop(
                    self.data_sum[
                        (self.data_sum["E"] >= self.emin_entry.get()) & (self.data_sum["E"] <= self.emax_entry.get())
                    ].index,
                    inplace=True,
                )
            ...
            if (
                (len(self.estartvar.get()) > 0)
                and (len(self.eendvar.get()) > 0)
                and (float(self.eendvar.get()) > float(self.estartvar.get()))
                and (float(self.estartvar.get()) < np.max(self.data["E"].values))
            ):
                self.data_sum = self.data_sum[
                    (self.data_sum["E"] >= float(self.estartvar.get()))
                    & (self.data_sum["E"] <= float(self.eendvar.get()))
                ]
    ...
    def saveaverage_trigger(self):
        self.data_sum
        self.data_sum.to_csv(
            self.folder + "/" + os.path.splitext(self.data_file)[0] + "_average_" + str(self.sumlen) + ".dat",
            sep="\t",
            index=False,
        )
        if self.errors_flag == True:
            self.data_sum_error.to_csv(
                self.folder + "/" + os.path.splitext(self.data_file)[0] + "_average_" + str(self.sumlen) + ".error",
                sep="\t",
                index=False,
            )
    ...
    def plot_spectrum(self):
        ...
        def plot_data(evt):
            ...
            if self.cutusevar.get() == 1:
                if self.AverageUsevar.get() == 0:
                    self.data.drop(
                        self.data[
                            (self.data["E"] >= self.emin_entry.get()) & (self.data["E"] <= self.emax_entry.get())
                        ].index,
                        inplace=True,
                    )
        ...
        if self.AverageUsevar.get() == 1:
            self.ds_scroll_scale = tkinter.Scale(
                ...
                orient="horizontal",
                from_=0,
                to=len(self.column_names_sum) - 2,
                state="normal",
                width=18,
                command=plot_data,
            )
        ...
        if self.trigger == 2:
            ...
            def scroll_bar_update(*args):
                self.pre1_scroll_scale = tkinter.Scale(
                    self.NormalisationFrame,
                    orient="horizontal",
                    from_=np.min(self.data_x) - self.e0_scroll_scale.get(),
                    to=0,
                    state="normal",
                    digits=4,
                    resolution=0.1,
                ...
                self.post1_scroll_scale = tkinter.Scale(
                    ...
                    orient="horizontal",
                    from_=0,
                    to=np.max(self.data_x) - self.e0_scroll_scale.get(),
                    state="normal",
                    digits=4,
                    resolution=0.1,
                    length=125,
    ...
    def savecut_trigger(self):
        if self.cutusevar.get() == 1:
            if self.NormUsevar.get() == 1:
                self.normalisation_values = (
                    pd.read_csv(self.folder + "/normalisation.dat", sep="\t", header=None)
                ).values
                self.pre1, self.pre2, self.post1, self.post2, self.order_pre, self.order_post, self.e0 = (
                    self.normalisation_values
                )
            ...
            self.export = self.export[
                (self.export["E"] >= np.float(self.emin_entry.get()))
                & (self.export["E"] <= np.float(self.emax_entry.get()))
            ]

            if self.NormUsevar.get() == 0:
                self.export.to_csv(
                    self.folder + "/" + os.path.splitext(self.data_file)[0] + "_cut.dat", sep="\t", index=False
                )
            else:
                self.export.to_csv(
                    self.folder + "/" + os.path.splitext(self.data_file)[0] + "_normalised_cut.dat",
                    sep="\t",
                    index=False,
                )
    ...
    def save_norm_data(self):
        ...
        if self.NormUsevar.get() == 1:
            ...
            if self.AverageUsevar.get() == 1:
                self.norm_export.to_csv(
                    self.folder
                    + "/"
                    + os.path.splitext(self.data_file)[0]
                    + "_normalised_"
                    + str(self.nsumvar.get())
                    + ".dat",
                    sep="\t",
                    index=False,
                )
                if self.errors_flag == True:
                    self.norm_export_error.to_csv(
                        self.folder
                        + "/"
                        + os.path.splitext(self.data_file)[0]
                        + "_normalised_"
                ...
                self.norm_export.to_csv(
                    self.folder + "/" + os.path.splitext(self.data_file)[0] + "_normalised.dat", sep="\t", index=False
                )
                if self.errors_flag == True:
                    self.norm_export_error.to_csv(
                        self.folder + "/" + os.path.splitext(self.data_file)[0] + "_normalised.error",
                        sep="\t",
                        index=False,
                    )
        ...
        if self.interpolated == 1:
            if self.mcrnormvar.get() == 1:
                ...
                self.norm_export.drop(
                    self.norm_export[self.norm_export["E"] >= self.emax_mcr.get()].index, inplace=True
                )
                self.norm_export.drop(
                    self.norm_export[self.norm_export["E"] <= self.emin_mcr.get()].index, inplace=True
                )
                ...
                if self.AverageUsevar.get() == 1:
                    ...
                    self.norm_export.to_csv(
                        self.folder + "/" + os.path.splitext(self.data_file)[0] + "_normalised_mcr.dat",
                        sep="\t",
                        index=False,
                        header=None,
                    )
...
class FourierTransform(tkinter.Frame):
    def __init__(self, name, *args, **kwargs):
        ...
        self.lclamp_scroll_scale = tkinter.Scale(
            self.xasbackFrame, orient="horizontal", from_=0, to=50, state="normal", width=18
        )
        ...
        self.hclamp_scroll_scale = tkinter.Scale(
            self.xasbackFrame, orient="horizontal", from_=0, to=50, state="normal", width=18
        )
        ...
        self.rbkg_scroll_scale = tkinter.Scale(
            self.xasbackFrame,
            orient="horizontal",
            from_=0.5,
            to=1.5,
        ...
        self.e0_scroll_scale = tkinter.Scale(
            self.xasbackFrame, orient="horizontal", from_=0, to=1, state="normal", width=18
        )
        ...
        self.kmin_scroll_scale = tkinter.Scale(
            self.FTFrame, orient="horizontal", from_=0, to=15, state="normal", width=18
        )
        ...
        self.kmax_scroll_scale = tkinter.Scale(
            self.FTFrame, orient="horizontal", from_=0, to=15, state="normal", width=18
        )
        ...
        self.label2 = tkinter.Label(self.FTFrame, text="", anchor="w").grid(
            column=0, row=4, columnspan=1, rowspan=1, sticky="S"
        )
        ...
        self.rmin_scroll_scale = tkinter.Scale(
            self.BFTFrame, orient="horizontal", from_=0, to=10, state="normal", width=18
        )
        ...
        self.rmax_scroll_scale = tkinter.Scale(
            self.BFTFrame, orient="horizontal", from_=0, to=10, state="normal", width=18
        )
        ...
        self.dR_scroll_scale = tkinter.Scale(
            self.BFTFrame, orient="horizontal", from_=0, to=4, state="normal", width=18
        )
        ...
        self.label2 = tkinter.Label(self.BFTFrame, text="", anchor="w").grid(
            column=0, row=3, columnspan=1, rowspan=1, sticky="S"
        )
    ...
    def plot_spectrum(self):
        ...
        def plot_data(evt):
            ...
            if self.initialize_plot == 0:
                ...
                autobk(
                    larch_data.energy,
                    larch_data.mu,
                    e0=self.e0_scroll_scale.get(),
                    rbkg=self.rbkg_scroll_scale.get(),
                    group=larch_data,
                    clamp_lo=self.lclamp_scroll_scale.get(),
                    clamp_hi=self.hclamp_scroll_scale.get(),
                    kweight=self.kwback.get(),
                    dk=self.dk_scroll_scale.get(),
                    kstep=self.Kstep,
                    _larch=mylarch,
                )
                ...
                xftf(
                    larch_data.k,
                    larch_data.chi,
                    kmin=self.kmin_scroll_scale.get(),
                    kmax=self.kmax_scroll_scale.get(),
                    dk=self.dk_scroll_scale.get(),
                    window="hanning",
                    kweight=self.kwft.get(),
                    group=larch_data,
                    _larch=mylarch,
                )
                xftr(
                    ...
                    larch_data.chir,
                    group=larch_data,
                    qmax_out=self.kmax_scroll_scale.get(),
                    rmin=self.rmin_scroll_scale.get(),
                    rmax=self.rmax_scroll_scale.get(),
                    window="hanning",
                    dr=self.dR_scroll_scale.get(),
                    _larch=mylarch,
                )

                self.kwin = ftwindow(
                    larch_data.k,
                    xmin=self.kmin_scroll_scale.get(),
                    xmax=self.kmax_scroll_scale.get(),
                    dx=self.dk_scroll_scale.get(),
                    dx2=self.dk_scroll_scale.get(),
                    window="hanning",
                )
                ...
                if self.errors_flag == True:
                    self.EKerror_data["kw_error"] = self.data_error[str(self.error_column_names[self.data_i + 1])] * (
                        self.EKerror_data["K"] ** self.kwft.get()
                    )

                self.EKerror_data = self.EKerror_data[(self.EKerror_data["E"] >= 0)].reset_index(drop=True)
                if self.errors_flag == True:
                    self.linek_errorbar = self.ax2.errorbar(
                        self.EKerror_data["K"],
                        self.EKerror_data["kw"],
                        yerr=self.EKerror_data["kw_error"],
                        ecolor="r",
                        capsize=3,
                        elinewidth=2,
                        markeredgewidth=1,
                    ...
                    [self.linek] = self.ax2.plot(
                        self.EKerror_data["K"], self.EKerror_data["kw"], linewidth=2, color="red"
                    )
                [self.line_kmin] = self.ax2.plot(
                    [self.kmin_scroll_scale.get(), self.kmin_scroll_scale.get()],
                    [-1, 1],
                    linewidth=2,
                    color="k",
                    ls="--",
                )
                [self.line_kmax] = self.ax2.plot(
                    [self.kmax_scroll_scale.get(), self.kmax_scroll_scale.get()],
                    [-1, 1],
                    linewidth=2,
                    color="k",
                    ls="--",
                ...
                self.rwin = ftwindow(
                    larch_data.r,
                    xmin=self.rmin_scroll_scale.get(),
                    xmax=self.rmax_scroll_scale.get(),
                    dx=self.dR_scroll_scale.get(),
                    dx2=self.dR_scroll_scale.get(),
                    window="hanning",
                )
                ...
                if self.errors_flag == True:
                    self.linek_errorbar = adjustErrbarxy(
                        self,
                        self.linek_errorbar,
                        self.EKerror_data["K"],
                        self.EKerror_data["kw"],
                        self.EKerror_data["kw_error"],
                    )
                else:
                ...
                if self.trigger != 0:
                    if self.data_i == 0:
                        ...
                        if self.errors_flag == True:
                            self.chiKwerror["k"] = np.sqrt(
                                (2 * mass_e * self.chiE["E"].values) / ((hbarc**2) * (10**8))
                            )
                    ...
                    if self.errors_flag == True:
                        self.chiKwerror[str(self.data_i)] = self.data_error[
                            str(self.error_column_names[self.data_i + 1])
                        ] * (self.chiKw["k"] ** self.kwft.get())
        ...
        if self.trigger == 0:
            ...
            if len(self.column_names) == 2:
                self.ds_scroll_scale = tkinter.Scale(
                    self.scrollFrame, orient="horizontal", from_=0, to=1, state="disabled", width=18, command=plot_data
                )
            else:
                self.ds_scroll_scale = tkinter.Scale(
                    self.scrollFrame,
            ...
            if self.initialize_plot == 0:
                self.kwback = tkinter.IntVar(self.xasbackFrame)
                self.kwbackMenu = tkinter.OptionMenu(
                    self.xasbackFrame, self.kwback, *self.kwbackchoices, command=plot_data
                )
                self.kwbackMenu.grid(column=1, row=0, columnspan=1)
                self.kwbacklabel = tkinter.Label(self.xasbackFrame, text="k weight", anchor="w")
                self.kwbacklabel.grid(column=0, row=0, columnspan=1, rowspan=1, sticky="S")
                ...
                self.lclamp_scroll_scale = tkinter.Scale(
                    self.xasbackFrame, orient="horizontal", from_=0, to=50, state="normal", width=18, command=plot_data
                )
                self.lclamp_scroll_scale.grid(column=1, columnspan=1, row=1, stick="EW")
                self.lclamplabel = tkinter.Label(self.xasbackFrame, text="Low Clamp", anchor="w")
                self.lclamplabel.grid(column=0, row=1, columnspan=1, rowspan=1, sticky="S")
                ...
                self.dk_scroll_scale = tkinter.Scale(
                    self.FTFrame,
                    orient="horizontal",
                    from_=0,
                    to=4,
        ...
        if self.trigger == 1:
            ...
            self.dfrm.to_csv(
                self.folder
                + "/FT/"
                + str(os.path.splitext(self.data_file)[0])
                + "/"
                + str(os.path.splitext(self.data_file)[0])
                + "_R-space_magnitude.dat",
                sep="\t",
                index=False,
            )
            self.dfkw.to_csv(
...
class Plotting(tkinter.Frame):
    ...
    def onplot(self):
        ...
        for i in selection:
            ...
            self.sliders[str(i)] = tkinter.Scale(
                self.frame3,
                orient="horizontal",
                from_=0,
                to=len(self.dataframe_collection[str(i)].columns) - 2,
                state="normal",
                width=18,
                sliderlength=10,
            )
    ...
    def plot_update_trigger(self, *args):
        for j in range(len(self.sliders)):
            if self.sliders[self.sliders_dict_keys[j]].get() != self.store_state[j]:
                ...
                data_y = self.dataframe_collection[self.sliders_dict_keys[j]].iloc[
                    :, int(self.sliders[self.sliders_dict_keys[j]].get()) + 1
                ]
...
class Analysis(tkinter.Frame):
    def __init__(self, name, *args, **kwargs):
        ...
        self.buttonsimplisma = tkinter.Button(
            self.AnalzeFrame, text="SIMPLISMA", width=40, height=1, command=self.simplisma
        )
        self.buttonsimplisma.grid(column=0, row=3, columnspan=2, rowspan=1)

        self.buttonsimplismasave = tkinter.Button(
            self.AnalzeFrame, text="Save", width=20, height=1, command=self.simplisma_save
        )
        ...
        self.simplisma_number_entry = tkinter.Entry(
            self.AnalzeFrame, textvariable=self.simplisma_number, state="normal"
        )
        self.simplisma_number_entry.grid(column=1, columnspan=2, row=4, stick="EW")

        self.buttonloadcomponents = tkinter.Button(
            self.AnalzeFrame, text="Load Components", width=20, height=1, command=self.loadcomponents
        )
        ...
        self.buttonmcrsave = tkinter.Button(
            self.AnalzeFrame, text="Save MCR", width=20, height=1, command=self.savemcrresults
        )
    ...
    def loadcomponents(self):
        ...
        try:
            ...
            for i in range(len(self.components_column_names) - 1):
                self.ax.plot(
                    self.components[self.components_column_names[0]],
                    self.components[self.components_column_names[i + 1]],
                )
    ...
    def svd(self):
        ...
        eigens, explained_variance_ratio = svd.svd(self.D, int(self.svd_number.get()))
    ...
    def simplisma_save(self):
        ...
        try:
            ...
            save_s.to_csv(
                self.folder + "\simplisma_components_" + str(self.simplisma_number.get()) + ".dat",
                sep="\t",
                index=False,
            )
        except:
    ...
    def lcfsave(self):
        ...
        Concentrations_df.to_csv(
            self.folder + "\lcf_concentrations_" + str(np.shape(self.S)[1]) + ".dat", sep="\t", index=False
        )
    ...
    def mcr(self):
        ...
        if (self.constrain_nn_usevar.get() == 1) & (self.constrain_us_usevar.get() == 1):
            self.mcrals = McrAls(
                max_iter=25, st_regr="NNLS", c_regr="NNLS", c_constraints=[ConstraintNonneg(), ConstraintNorm()]
            )
            print("contraining norm and non negativity")
        if (self.constrain_nn_usevar.get() == 1) & (self.constrain_us_usevar.get() == 0):
            self.mcrals = McrAls(max_iter=25, st_regr="NNLS", c_regr="NNLS", c_constraints=[ConstraintNonneg()])
            print("contraining non negativity")
        if (self.constrain_nn_usevar.get() == 0) & (self.constrain_us_usevar.get() == 1):
            self.mcrals = McrAls(max_iter=25, st_regr="NNLS", c_regr="NNLS", c_constraints=[ConstraintNorm()])
            print("contraining norm")
        if (self.constrain_nn_usevar.get() == 0) & (self.constrain_us_usevar.get() == 0):
            self.mcrals = McrAls(max_iter=25, st_regr="NNLS", c_regr="NNLS", c_constraints=[])
            print("no contraints")
    ...
    def savemcrresults(self):
        ...
        Concentrations_df["LOF"] = self.LOF
        Concentrations_df.to_csv(
            self.folder + "\mcr_concetrations_" + str(np.shape(self.mcrals.ST_opt_)[0]) + ".dat", sep="\t", index=True
        )
        ...
        try:
            Spectra_df.insert(0, "Energy", self.energy)
            Spectra_df.to_csv(
                self.folder + "\mcr_spectra_" + str(np.shape(self.mcrals.ST_opt_)[0]) + ".dat", sep="\t", index=False
            )
        except:
            Spectra_df.to_csv(
                self.folder + "\mcr_spectra_" + str(np.shape(self.mcrals.ST_opt_)[0]) + ".dat", sep="\t", index=False
